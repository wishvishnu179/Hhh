...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            ...
            'metadata': ['Leech Filename Metadata is the Front Part attacted with the Filename of the Mirrored/Cloned Files.', 'Send Mirror Filename METADATA. \n<b>Timeout:</b> 60 sec'],
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        elif key in ['mprefix', 'metadata', 'mremname', 'msuffix']:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'mprefix', 'metadata', 'msuffix', 'mremname']:
        handler_dict[user_id] = False
        await query.answer()
        edit_mode = len(data) == 4
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['dlprefix', 'dlmetadata', 'dlsuffix', 'dlremname', 'dlcaption', 'dldump']:
        handler_dict[user_id] = False
        await query.answer()
        ...
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['dmprefix', 'dmetadata', 'dmsuffix', 'dmremname', 'duser_tds']:
        handler_dict[user_id] = False
        await query.answer()
